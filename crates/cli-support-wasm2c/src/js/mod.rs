...
const WASM_MEM_CODE: &'static str = r"
...
typedef struct WASM_RT_ADD_PREFIX(wasm_slice_ptr) {
    u32 address;
    u32 length;
...
static inline
WASM_RT_ADD_PREFIX(wasm_slice_ptr_t)
WASM_RT_ADD_PREFIX(wasm_slice_ptr_from_address)(u32 address, u32 length) {
...
static inline
u32
WASM_RT_ADD_PREFIX(wasm_slice_ptr_get_address)(
...
static inline
u32
WASM_RT_ADD_PREFIX(wasm_slice_ptr_get_length)(
    WASM_RT_ADD_PREFIX(wasm_slice_ptr_t) ptr)
{
    return ptr.length;
}
...
static inline
bool
WASM_RT_ADD_PREFIX(wasm_slice_ptr_is_null)(
    WASM_RT_ADD_PREFIX(wasm_slice_ptr_t) ptr)
{
...
static inline
void*
WASM_RT_ADD_PREFIX(wasm_slice_ptr_get_ptr)(
    WASM_RT_ADD_PREFIX(wasm_slice_ptr_t) ptr)
{
    if (WASM_RT_ADD_PREFIX(wasm_slice_ptr_is_null)(ptr)) {
        ...
        return (WASM_RT_ADD_PREFIX(Z_memory)->data + ptr.address);
    }
}
    ...
    WASM_RT_ADD_PREFIX(wasm_slice_ptr_t) ptr)
{
    ...
    WASM_RT_ADD_PREFIX(Z___wbindgen_freeZ_vii)(
        ptr.address,
        ptr.length
    );
}
...
static inline
u32
WASM_RT_ADD_PREFIX(utf8_to_utf32)(const char* s) {
    u32 ch = 0;
    int i = 0;
    if (((u8)s[0]) < 128) {
        /* 1 byte */
        ...
        if (!(((u8)s[0]) & 0x1E) || ((((u8)s[1]) & 0xC0) != 0x80)) {
            return 0; /* err */
        }

        ch = (u32)((((u8)s[0]) & 0x1E) << 6) + (u32)(((u8)s[1]) & 0x3F);
    }
        ...
        if (!(((u8)s[0]) & 0x0F) && !(((u8)s[1]) & 0x20)) {
            return 0; /* err */
        }

        if (((((u8)s[1]) & 0xC0) != 0x80) || ((((u8)s[2]) & 0xC0) != 0x80)) {
            return 0; /* err */
        }

        ch = (u32)((((u8)s[0]) & 0x0F) << 12) + (u32)((((u8)s[1]) & 0x3F) << 6) + (u32)(((u8)s[2]) & 0x3F);
    }
    else if ((((u8)s[0]) & 0xF8) == 0xF0) {
        /* 4 bytes */
        if (s[1] == 0 || s[2] == 0 || s[3] == 0) {
            return 0; /* err */
        }

        if (!(((u8)s[0]) & 0x07) && !(((u8)s[1]) & 0x30)) {
            return 0; /* err */
        }

        if (((((u8)s[1]) & 0xC0) != 0x80) || ((((u8)s[2]) & 0xC0) != 0x80) || ((((u8)s[3]) & 0xC0) != 0x80)) {
            return 0; /* err */
        }

        ch = (u32)((((u8)s[0]) & 0x07) << 18) + (u32)((((u8)s[1]) & 0x3F) << 12) + (u32)((((u8)s[2]) & 0x3F) << 6)+ (u32)(((u8)s[3]) & 0x3F);
    }
    else {
        return 0; /* err */
    }
...
static inline
WASM_RT_ADD_PREFIX(wasm_slice_ptr_t)
WASM_RT_ADD_PREFIX(utf32_to_utf8)(u32 ch) {
    WASM_RT_ADD_PREFIX(wasm_slice_ptr_t) ptr = {0};
    if (ch < 128) {
        u8 s[2] = { (u8)ch, 0 };
        ptr = WASM_RT_ADD_PREFIX(wasm_slice_ptr_alloc)(sizeof(s));
        WASM_RT_ADD_PREFIX(wasm_slice_ptr_memwrite)(ptr, s, sizeof(s));
        return ptr;
    }
    else if (ch < 2048) {
        u8 s[3] = { 0xC0 | (u8)((ch & 0x7C0) >> 6), 0x80 | (u8)(ch & 0x3F), 0 };
        ptr = WASM_RT_ADD_PREFIX(wasm_slice_ptr_alloc)(sizeof(s));
        WASM_RT_ADD_PREFIX(wasm_slice_ptr_memwrite)(ptr, s, sizeof(s));
        return ptr;
    }
    else if (ch < 65536) {
        u8 s[4] = { 0xE0 | (u8)((ch & 0xF000) >> 12), 0x80 | (u8)((ch & 0xFC0) >> 6), 0x80 | (u8)(ch & 0x3F), 0 };
        ptr = WASM_RT_ADD_PREFIX(wasm_slice_ptr_alloc)(sizeof(s));
        WASM_RT_ADD_PREFIX(wasm_slice_ptr_memwrite)(ptr, s, sizeof(s));
        return ptr;
    }
    else if (ch < 0x110000) {
        u8 s[5] = { 0xF0 | (u8)((ch & 0x1C0000) >> 18), 0x80 | (u8)((ch & 0x3F000) >> 12), 0x80 | (u8)((ch & 0xFC0) >> 6), 0x80 | (u8)(ch & 0x3F), 0 };
        ptr = WASM_RT_ADD_PREFIX(wasm_slice_ptr_alloc)(sizeof(s));
        WASM_RT_ADD_PREFIX(wasm_slice_ptr_memwrite)(ptr, s, sizeof(s));
        return ptr;
    }
...
impl<'a> Context<'a> {
    ...
    fn export(
            ...
            | OutputMode::Wasm2c => {
                if contents.starts_with("function") {
                    ...
                    if export_name == definition_name {
                        ...
                        format!("static inline {} WASM_RT_ADD_PREFIX({}){}\n", ret_type, export_name, body)
                    } else {
                        format!(
                            "function {}{}\nexport {{ {} as {} }};\n",
        ...
        if global.starts_with("typedef struct") {
            let g = if let Some(c) = comments {
                format!("{}{}", c, global)
            }
            else {
                global
    ...
    fn finalize_js(
        ...
        match &self.config.mode {
            ...
            OutputMode::NoModules { global } => {
                ...
                init = ("".into(), "".into());//self.gen_init(needs_manual_start, None)?;
            ...
            OutputMode::Web | OutputMode::Wasm2c => {
                ...
                self.imports_post.push_str("#include <string.h>\n");
                self.imports_post.push_str(WASM_MEM_CODE);
                //self.imports_post.push_str("let wasm;\n");
                init = ("".into(), "".into());//self.gen_init(needs_manual_start, Some(&mut imports))?;
    ...
    fn write_class(&mut self, name: &str, class: &ExportedClass) -> Result<(), Error> {
        let mut dst = format!("typedef struct WASM_RT_ADD_PREFIX({}_s) {{\n", name);
        let mut ts_dst = format!("export {}", dst);

        dst.push_str("    u32 ptr;\n");
        dst.push_str(&format!("}} WASM_RT_ADD_PREFIX({});\n\n", name));

        dst.push_str(&format!("static inline void WASM_RT_ADD_PREFIX({0}_free)(WASM_RT_ADD_PREFIX({0}) self) {{\n", name));
        dst.push_str(&format!("    (WASM_RT_ADD_PREFIX(Z_{}Z_vi))(self.ptr);\n", wasm_bindgen_shared::free_function(&name)));
        dst.push_str(&format!("}}\n\n"));
                            ...
                            &format!("function{}###{}",
                                ...
                                if adapter.results.len() > 0 {
                                    match &adapter.results[0] {
                                        AdapterType::Vector(_) => "???".to_owned(),
                                        AdapterType::S8 => "s8".to_owned(),
                                        AdapterType::S16 => "s16".to_owned(),
                                        AdapterType::S32 => "s32".to_owned(),
                                        AdapterType::S64 => "s64".to_owned(),
                                        AdapterType::U8 => "u8".to_owned(),
                                        AdapterType::U16 => "u16".to_owned(),
                                        AdapterType::U32 => "u32".to_owned(),
                                        AdapterType::U64 => "u64".to_owned(),
                                        AdapterType::F32 => "f32".to_owned(),
                                        AdapterType::F64 => "f64".to_owned(),
                                        AdapterType::String => "WASM_RT_ADD_PREFIX(wasm_slice_ptr_t)".to_owned(),
                                        AdapterType::Struct(s) => s.to_owned(),// ???
                                        /* not supported yet.....
                                        AdapterType::Externref => {},
                                        */
                                        AdapterType::Bool => "bool".to_owned(),
                                        AdapterType::I32 => "s32".to_owned(),
                                        /*
                                        AdapterType::I64 => {},
                                        */
                                ...
                                match &ts_ret_ty { // HACK: set return type
                                    Some(s) => {
                                        if s == "number" {
                                            "u32"
                                        }
    ...
    fn generate_enum(&mut self, enum_: &AuxEnum) -> Result<(), Error> {
        ...
        for (name, value, comments) in enum_.variants.iter() {
            variants.push_str(&format!("WASM_RT_ADD_PREFIX({}_{}) = {},\n", enum_.name, name, value));
            /*
        ...
        self.export(
            &enum_.name,
            &format!("typedef enum WASM_RT_ADD_PREFIX({0}_e) {{\n{1}}} WASM_RT_ADD_PREFIX({0});\n", enum_.name, variants),
            Some(&docs),
        )?;
